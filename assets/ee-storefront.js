...
function renderStoreAisles(){
  ...
  const promo=EE_CATALOG.find(p=>p.category==='Hair Care'&&p.image&&p.available!==false)||EE_CATALOG.find(p=>p.image)||EE_CATALOG[0];
...
function addToCart(id,qty=1){const p=EE_CATALOG.find(x=>x.id===id);if(!p)return;if(!(Number(p.price)>0)){toast('Available in store—ask our team for current pricing.');return}const item=state.cart.find(x=>x.id===id);item?item.qty+=qty:state.cart.push({id,qty});persist();renderCart();toast(`${p.brand} added to cart`)}
function renderCart(){const count=state.cart.reduce((n,i)=>n+i.qty,0);$('#cartCount').textContent=count;$('#mobileCartCount').textContent=count;const items=$('#cartItems');if(!state.cart.length){items.innerHTML='<p style="text-align:center;color:var(--muted);padding:48px 12px">Your cart is empty.<br><br><a class="btn btn-secondary" href="#catalog" data-close>Start Shopping</a></p>'}else{items.innerHTML=state.cart.map(i=>{const p=EE_CATALOG.find(x=>x.id===i.id);if(!p)return'';const image=assetUrl(p.image)||categoryFallback(p.category);return `<div class="cart-item"><img src="${esc(withWidth(image,400))}" alt="${esc(p.title)}" onerror="this.onerror=null;this.src=ASSETS.flat"><div><strong>${esc(p.title)}</strong><small>${esc(p.brand)}</small><div class="qty"><button data-qty="${p.id}" data-delta="-1">−</button><span>${i.qty}</span><button data-qty="${p.id}" data-delta="1">+</button></div><button class="remove" data-remove="${p.id}">Remove</button></div><b>${money(p.price*i.qty)}</b></div>`}).join('')}const cartTotalNum=state.cart.reduce((s,i)=>{const p=EE_CATALOG.find(x=>x.id===i.id);return s+(p?p.price*i.qty:0)},0);$('#cartTotal').textContent=money(cartTotalNum);updateLayawayCalc(cartTotalNum)}
...
async function eeStartShopifyCheckout(){
  ...
  try{
    const rows=state.cart.map(i=>({item:i,product:EE_CATALOG.find(p=>p.id===i.id)})).filter(r=>r.product&&r.product.handle);
    ...
    const total=state.cart.reduce((s,i)=>{const p=EE_CATALOG.find(x=>x.id===i.id);return s+(p?p.price*i.qty:0)},0);
...
quickView=function(id){const p=EE_CATALOG.find(x=>x.id===id);if(p){eeView={p:p,t0:Date.now()}}return _quickView(id)};
...
document.addEventListener('click',e=>{
const add=e.target.closest('[data-add]');if(add){const p=EE_CATALOG.find(x=>x.id===add.dataset.add);if(p)eeTrack('add_to_cart',itemParams(p));return}
const wish=e.target.closest('[data-wish]');if(wish){const p=EE_CATALOG.find(x=>x.id===wish.dataset.wish);if(p)eeTrack('add_to_wishlist',itemParams(p));return}
...
eeStartShopifyCheckout = async function(){
  ...
  try{
    ...
    for(const line of state.cart){
      const p=EE_CATALOG.find(x=>x.id===line.id);
...
/* Native Shopify AJAX cart — collection/product cards + drawer sync */
(function(){
  function moneyShopify(cents){
    const fmt = window.EE_MONEY_FORMAT || '${{amount}}';
    const amount = (Number(cents||0)/100).toFixed(2);
    return fmt.replace('{{amount}}', amount).replace('{{amount_no_decimals}}', String(Math.round(cents/100)));
  }
  ...
  function paintShopifyCart(cart){
    if(!cart) return;
    const count = cart.item_count || 0;
    const countEls = [$('#cartCount'), $('#mobileCartCount')];
    countEls.forEach(el => { if(el) el.textContent = count; });
    const items = $('#cartItems');
    if(!items) return;
    if(!cart.items || !cart.items.length){
      items.innerHTML = '<p style="text-align:center;color:var(--muted);padding:48px 12px">Your cart is empty.<br><br><a class="btn btn-secondary" href="'+(window.EE_ROUTES&&EE_ROUTES.collections||'/collections/all')+'" data-close>Start Shopping</a></p>';
    } else {
      items.innerHTML = cart.items.map(item => {
        const img = (item.image || '').replace(/(\.[a-z]+)(\?|$)/i, '_400x$1$2');
        return `<div class="cart-item"><img src="${esc(img)}" alt="${esc(item.product_title)}"><div><strong>${esc(item.product_title)}</strong><small>${esc(item.variant_title||item.vendor||'')}</small><div class="qty"><span>${item.quantity}</span></div><a class="remove" href="/cart/change?line=${item.key}&quantity=0">Remove</a></div><b>${moneyShopify(item.final_line_price)}</b></div>`;
      }).join('');
    }
    const total = $('#cartTotal');
    if(total) total.textContent = moneyShopify(cart.total_price);
    updateLayawayCalc((cart.total_price||0)/100);
  }
  window.eeRefreshShopifyCart = async function(){
    const cart = await shopifyCart();
    if(cart) paintShopifyCart(cart);
    return cart;
  };
  document.addEventListener('submit', function(e){
    const form = e.target.closest('form.js-product-form, form[action*="/cart/add"]');
    if(!form || form.id === 'eeProductForm') return;
    ...
    fetch((window.EE_ROUTES && EE_ROUTES.cartAdd) || '/cart/add.js', {
      ...
      return window.eeRefreshShopifyCart();
    }).then(function(){
      toast('Added to cart');
  ...
  if(!window.EE_IS_INDEX){
    window.eeRefreshShopifyCart();
  }
